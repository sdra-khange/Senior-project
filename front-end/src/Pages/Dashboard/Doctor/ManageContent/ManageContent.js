...
export default function ManageContent() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch content types from Content model
            const types = [...new Set(contentsRes.data.map(c => c.ContentType?.TypeName).filter(Boolean))];
            setContentTypes(types.map(type => ({ TypeName: type })));
        } catch (error) {
            console.error("Error fetching data:", error);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (currentContent.ContentID) {
                await axios.put(`/app/content/${currentContent.ContentID}/`, formData);
            } else {
                await axios.post('/app/content/', formData);
            }
            setShowForm(false);
    ...
    const handleEdit = (content) => {
        setCurrentContent({
            ...content,
            ContentType: content.ContentType?.TypeID || "",
            Domain: content.Domain?.DomainID || ""
        });
        setShowForm(true);
    };
    ...
    return (
        <div className="doctor-page">
            ...
            <div className="content-wrapper">
                ...
                {showForm && (
                    <div className="content-form">
                        <h2>{currentContent.ContentID ? "Edit Content" : "Add New Content"}</h2>
                        <form onSubmit={handleSubmit}>
                            <div className="form-group">
                                <label>Title</label>
                                <input
